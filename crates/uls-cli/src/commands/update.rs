...
use uls_download::{
    DownloadConfig, DownloadProgress, DownloadResult, FccClient, ProgressCallback, ServiceCatalog,
};
...
pub async fn execute(service: &str, force: bool, minimal: bool) -> Result<()> {
    ...
    let needs_import = match download_result {
        ...
        DownloadResult::NotModified => {
            println!(
                "\n✓ No new {} data available (FCC file unchanged).",
                service
            );
            if force {
    ...
    let stats =
        importer.import_for_service(&zip_path, service_code, import_mode, Some(progress_cb))?;
    ...
    println!(
        "✓ Imported {} records from {} files in {:.1}s ({:.0} records/sec)",
        stats.records,
        stats.files,
        stats.duration_secs,
        stats.rate()
    ...
    if stats.parse_errors > 0 || stats.insert_errors > 0 {
        ...
        if stats.parse_errors > 0 {
            println!(
                "⚠ {} parse errors (use -v to see details)",
                stats.parse_errors
            );
        }
        if stats.insert_errors > 0 {
            println!(
                "⚠ {} insert errors (use -v to see details)",
                stats.insert_errors
            );
        }
    }
