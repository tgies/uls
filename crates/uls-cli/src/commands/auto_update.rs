//! Auto-update functionality for lazy loading of data.
...
use uls_download::{
    DownloadConfig, DownloadProgress, FccClient, ProgressCallback as DownloadCallback,
    ServiceCatalog,
};
...
pub async fn ensure_data_available(service: &str) -> Result<Database> {
    ...
    let stats =
        importer.import_for_service(&zip_path, service, ImportMode::Minimal, Some(progress_cb))?;
