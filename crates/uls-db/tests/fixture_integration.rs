...
fn test_amateur_fixture_query_callsigns() {
    ...
    for callsign in callsigns.iter().take(5) {
        if let Some(license) = db.get_license_by_callsign(callsign).unwrap() {
            ...
            assert!(license.radio_service == "HA" || license.radio_service == "HV",
                "Expected amateur service for {}, got {}", callsign, license.radio_service);
            found += 1;
        }
    }
...
fn test_amateur_fixture_entity_data() {
    ...
    if let Some(callsign) = callsigns.first() {
        if let Some(license) = db.get_license_by_callsign(callsign).unwrap() {
            ...
            assert!(!name.is_empty(), "Should have licensee name for {}", callsign);
        }
    }
}
...
fn test_gmrs_fixture_query_callsigns() {
    ...
    for callsign in callsigns.iter().take(5) {
        if let Some(license) = db.get_license_by_callsign(callsign).unwrap() {
            assert_eq!(&license.call_sign, callsign);
            assert_eq!(license.radio_service, "ZA",
                "Expected GMRS (ZA) service for {}, got {}", callsign, license.radio_service);
            found += 1;
        }
    }
...
fn test_callsign_formats_are_correct() {
    ...
    for cs in amat_callsigns.iter().take(10) {
        ...
        if let Some(pos) = digit_pos {
            ...
            assert!(after_digit.iter().any(|c| c.is_ascii_alphabetic()),
                "Amateur callsign {} should have letters after digit", cs);
        } else {
    ...
    for cs in gmrs_callsigns.iter().take(10) {
        ...
        if let Some(pos) = first_digit {
            ...
            assert!(chars[..pos].iter().all(|c| c.is_ascii_alphabetic()),
                "GMRS callsign {} prefix should be all letters", cs);
            // Everything after (including pos) should be digits
            assert!(chars[pos..].iter().all(|c| c.is_ascii_digit()),
                "GMRS callsign {} suffix should be all digits", cs);
        } else {
...
fn test_combined_amateur_and_gmrs_import() {
    ...
    assert!(found_amat, "Should find amateur callsigns after combined import");
    assert!(found_gmrs, "Should find GMRS callsigns after combined import");
}
